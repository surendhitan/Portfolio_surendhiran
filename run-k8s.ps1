...
function Check-Prerequisites {
  ...
  foreach ($tool in $tools) {
    if (-not (Get-Command $tool -ErrorAction SilentlyContinue)) {
      Write-Host "  [X] '$tool' not found. Please install it first." -ForegroundColor Red
      exit 1
    }
...
function Start-Minikube {
  ...
  Write-Host "  [OK] Addons enabled (ingress, ingress-dns, metrics-server)" -ForegroundColor Green
}
...
function Create-TLSSecret {
  Write-Host "`nCreating TLS secret..." -ForegroundColor Yellow
  ...
  kubectl create secret tls portfolio-tls-secret `
    ...
    --key="$KEY" `
    --namespace=$NAMESPACE
  Write-Host "  [OK] TLS secret created from: $CERT" -ForegroundColor Green
}

function Deploy-Manifests {
  Write-Host "`nApplying Kubernetes manifests..." -ForegroundColor Yellow
  kubectl apply -f "$K8S_DIR\deployment.yaml"
  Write-Host "  [OK] Deployment, Service, Ingress, HPA applied" -ForegroundColor Green
}
...
function Open-Site {
  Write-Host "`nOpening https://$DOMAIN ..." -ForegroundColor Yellow
  $ip = minikube ip
...
function Teardown {
  Write-Host "`nTearing down portfolio deployment..." -ForegroundColor Red
...
if ($Clean) {
  Teardown
  Write-Host "`n[OK] Clean teardown complete." -ForegroundColor Green
  exit 0
}
...
Write-Host "    Namespace    : portfolio                          " -ForegroundColor Green
